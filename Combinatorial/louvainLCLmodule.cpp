...
static PyObject* stability(PyObject* self, PyObject* args) {
    ...
    if (!PyArg_ParseTuple(args, "Odd" , &input, &timet, &precision))	//&weighted
        return NULL;
	...
	// double out;
	// out = (double)n2c[4];
	// pointer_to_data[0] = out;
	// out2 = pointer_to_data[0];
...
PyMODINIT_FUNC PyInit_louvainLCL (void)
{
    PyObject *module;
	static struct PyModuleDef louvainLCLmoduledef = {
		PyModuleDef_HEAD_INIT,
		"louvainLCL", /*name of module */
		NULL, /*module documentation, need to fill this in*/
		-1,
		methods
};

	module =  PyModule_Create(&louvainLCLmoduledef);
    import_array();
...
/*
inputs:
0 - graph as edge list [col, row, val] = data and length_data
1 - value of 1 corresponding to 'timet'
2 - precision of calculation
3 - 'w' is weighted (ascii 119) or 'u' is unweighted (ascii 117)

outputs:
tuple of (stability, no. of communities, vector of community id's')
i.e (double, int, numpy array)
